...
router.get('/', async (req, res) => {
    ...
    res.render('mangahomepage',
        { user, shows: usershows }
    )
})
router.get('/watchlist', async (req, res) => {
    const user = req.user || null;
    const usershows = user ? await shows.find({ createdBy: user._id, type: "MANGA", status: "WATCHLIST" }).sort({ rating: -1 }) : [];
    res.render('mangawatchlist', { user, shows: usershows })
})
router.get('/finish', async (req, res) => {
    const user = req.user || null;
    const usershows = user ? await shows.find({ createdBy: user._id, type: "MANGA", status: "FINISHED" }).sort({ rating: -1 }) : [];
    res.render('mangafinish', { user, shows: usershows })
})
...
router.post('/addtofinish', async (req, res) => {
    try {
        ...
        const show = await shows.findOne({ _id: objectId, type: "MANGA" });
